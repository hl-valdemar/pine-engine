...
const WorldState = struct {
    const cube_label: []const u8 = "basic-cube-example";
    ...
    pub fn run(self: *WorldState) void {
        sokol.app.run(sokol.app.Desc{
            .init_userdata_cb = sokolInitBasicCubeExample,
            .frame_userdata_cb = sokolFrameBasicCubeExample,
            .event_userdata_cb = sokolEventBasicCubeExample,
            .user_data = self,
            ...
            .width = 4 * 300,
            .height = 3 * 300,
            .window_title = "Pine: Basic Cube Example",
        });
    }

    export fn sokolInitBasicCubeExample(world_state: ?*anyopaque) void {
    ...
    export fn sokolFrameBasicCubeExample(world_state: ?*anyopaque) void {
        if (world_state) |state| {
            const self: *WorldState = @alignCast(@ptrCast(state));
    ...
    export fn sokolEventBasicCubeExample(ev: [*c]const sokol.app.Event, world_state: ?*anyopaque) void {
        if (world_state) |state| {
            const self: *WorldState = @alignCast(@ptrCast(state));
